...
const handleSpeech = () => {
  if (speechRecognitionSupported) {
    ...
    const recognition = new (window.SpeechRecognitionEvent || window.webkitSpeechRecognitionEvent)();
    ...
    navigator.mediaDevices.getUserMedia({ audio: true })
      ...
      .catch((error) => {
        ...
        alert('Error accessing microphone. Please make sure your browser settings allow microphone access.');
      });
...
useEffect(() => { if ('SpeechRecognition' in window || 'webkitSpeechRecognition' in window) { setSpeechRecognitionSupported(true); } }, []);
    ...
    <div className="grid p-2 static sm:grid grid-flow-col shadow-xl px-4 items-center">
      ...
      <div className="flex sm:flex items-center justify-center col-span-8 text-center">
        <input
          ...
          className="px-2 sm:px-4 py-2 w-3/6 rounded-l-3xl border-2 border-gray-700 outline-none bg-[#0f0f0f] text-white"
        />
        ...
        <a href={`/search_results/${search}`}>
          <button className="px-4 py-2 font-semibold border-2 border-gray-800 bg-[#181818] text-white rounded-r-3xl text-center items-center hover:bg-[#0f0f0f]">
            <svg
              stroke="currentColor"
              fill="currentColor"
              viewBox="0 0 512 512"
        ...
        <div onClick={handleSpeech} className="hidden lg:flex justify-center items-center rounded-3xl cursor-pointer text-lg mx-2 w-10 h-10 bg-[#181818] hover:bg-[#0f0f0f]">
          <FaMicrophone />
        </div>
      </div>
...
//       recognition.onstart = () => console.log('Microphone permission granted');
...
//         {showSuggestion && (
//           <div className="hidden mt-14 ml-[-8%] rounded-md md:absolute md:w-[35%] md:block">
//             <div className="absolute px-4 py-2 w-full bg-green-400 rounded-md">
